...
% April, 2019
% Daniele Porello
% Contributions from Giancarlo Guizzardi, Claudenir M. Fonseca, Nicola Guarino
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%Proved consistent Darwin 1.4.4.
                ...
                %%%%%%%%%%%%%%%%Intrinsic property.
                %%Simplifying assumption: only intrinsic moments that are not externally dependent modes are truthmakers of intrinsic properties. However, some types are intrinsic properties? E.g. being human.


              fof(ax_def_intrinsicP, axiom, (![P]: (intrinsicproperty(P) <=>
                (property(P) & ?[W,X]:(iof(X,P,W)) & (![X,W]:(iof(X,P,W) <=> ?[M]:((intrinsicmoment(M) & ~edm(M) & inheresIn(M,X))))))))).
                ...
                %%Just not intrinsic.
                        ...
                        %%%%%%%%%%%%%%%%Descriptive properties.
                                %%Are those that are true of their instances because of the existence of a (general) intrinsic moment that inheres in the instances
                                %(it may be a mode, a quality and also an externally dependent mode).

                                fof(ax_def_descriptiveP, axiom, (![P]: (descriptiveproperty(P) <=> (property(P) & ?[W,X]:(iof(X,P,W)) & ![X,W]: (iof(X,P,W) <=>
                                    ?[M]: (intrinsicmoment(M) & inheresIn(M,X))))))).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%Derive Relation
%%it serves to associate a relation with the pertinent moment that are required for the truth making.

fof(ax_def_derive, axiom, (![R,X]: (derive(R,X) => (relation(R) & (momenttype(X) | relation(X)))))).
...
%% This is weak truth making; strong truthmaking requires more information, e.g. comparative relations "taller than" besides the qualities "height", another relation of "greater than" is required.
...
?[P,Q,M]:(intrinsicproperty(P) & (intrinsicproperty(Q) & momenttype(M) &  derive(R,M) & iof(P,M,W2) & iof(Q,M,W2) & (iof(X,P,W2) & iof(Y,Q,W2))))))))))).
...
%% Descriptive relations are those that are true because of
...
% ii) the existence of two intrinsic properties of the relata.
    ...
    (relation(R) & (?[W1,X,Y] : (world(W1) & iofr(X,Y,R,W1)) =>
        (![W2,X,Y] : (iofr(X,Y,R,W2) <=>
            ((?[M,T]: (edm(M) & momenttype(T) & derive(R,T) & iof(M,T,W2) & ex(M,W2) & ((inheresIn(M,X) & ed(M,Y)) | (inheresIn(M,Y) & ed(M,X)))))
               | (?[P,Q,T]:(intrinsicproperty(P) & intrinsicproperty(Q) & momenttype(T) & derive(R,T) & iof(P,T,W2) & iof(Q,T,W2) & iof(X,P,W2) & iof(Y,Q,W2))))
))))))).
